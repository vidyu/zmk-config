...
_build_single $board $shield $snippet $artifact *west_args:
    ...
    west build -s zmk/app -d "$build_dir" -b $board {{ west_args }} ${snippet:+-S "$snippet"} -- \
        -DZMK_CONFIG="{{ config }}" -DBOARD_ROOT="{{ justfile_directory() }}" ${shield:+-DSHIELD="$shield"}
...
preview:
    #!/usr/bin/env bash
    if [[ -f "{{ draw }}/base.svg" ]]; then
        qlmanage -p "{{ draw }}/base.svg"
    else
        echo "No keymap found. Run 'just draw' first to generate the SVG."
        exit 1
    fi
